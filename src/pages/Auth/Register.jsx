...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (res.success) {
      if (['equipment_owner', 'mart_owner', 'factory_owner'].includes(res.role)) navigate('/store-dashboard')
  ...
  const districtKeys = Object.keys(maharashtraData).filter(k => 
    maharashtraData[k][language].toLowerCase().includes(distSearch.toLowerCase()) ||
  ...
  const talukaList = form.district ? maharashtraData[form.district].talukas.filter(t => 
    t[language].toLowerCase().includes(talSearch.toLowerCase()) ||
                  ...
                  <div className="grid grid-cols-2 gap-5">
                    <div className="space-y-2 relative">
                      ...
                      <button type="button" onClick={() => {setDistOpen(!distOpen); setTalOpen(false)}} 
                        ...
                        <span className="truncate">{form.district ? maharashtraData[form.district][language] : 'Select'}</span>
                      ...
                      {distOpen && (
                        <div className="absolute bottom-full left-0 right-0 mb-3 bg-white border border-emerald-200 rounded-3xl shadow-2xl z-[70] overflow-hidden max-h-56 flex flex-col w-[130%]">
                          ...
                          <div className="overflow-y-auto flex-1">
                            {districtKeys.map(k => <button key={k} type="button" onClick={() => update('district', k)} className="w-full text-left px-7 py-4 text-sm font-bold hover:bg-emerald-50 text-emerald-950 transition-colors border-b border-emerald-50 last:border-0">{maharashtraData[k][language]}</button>)}
                          </div>
                        </div>
                      )}
                    </div>
                    <div className="space-y-2 relative">
                      ...
                      <button type="button" disabled={!form.district} onClick={() => {setTalOpen(!talOpen); setDistOpen(false)}} 
                        ...
                        <span className="truncate">{form.taluka ? (maharashtraData[form.district].talukas.find(t => t.en === form.taluka)?.[language]) : 'Select'}</span>
                        <ChevronDown size={18} className="text-emerald-300" />
                      </button>
                      {talOpen && (
                        <div className="absolute bottom-full right-0 mb-3 bg-white border border-emerald-200 rounded-3xl shadow-2xl z-[70] overflow-hidden max-h-56 flex flex-col w-[130%]">
                          <input autoFocus type="text" placeholder="Search Taluka..." value={talSearch} onChange={e => setTalSearch(e.target.value)} 
                            className="p-5 text-sm border-b border-emerald-100 outline-none font-bold placeholder:text-emerald-200" />
                          <div className="overflow-y-auto flex-1">
                            {talukaList.map(t => <button key={t.en} type="button" onClick={() => update('taluka', t.en)} className="w-full text-left px-7 py-4 text-sm font-bold hover:bg-emerald-50 text-emerald-950 transition-colors border-b border-emerald-50 last:border-0">{t[language]}</button>)}
                          </div>
                        </div>
                      )}
                    </div>
